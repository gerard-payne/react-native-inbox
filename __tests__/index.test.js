...
describe('React Native IMAP', () => {
  ...
  describe('downloadEmails', () => {
    it('should call native downloadEmails method with correct parameters', async () => {
      const options = {
        folder: 'INBOX',
        limit: 10,
      };
      ...
      RNImap.downloadEmails.mockResolvedValue(mockEmails);

      const result = await Imap.downloadEmails(options);

      expect(RNImap.downloadEmails).toHaveBeenCalledWith(options);
      expect(result).toEqual(mockEmails);
    });

    it('should throw error when native downloadEmails fails', async () => {
      const options = {
        folder: 'INBOX',
        limit: 10,
      };
      ...
      RNImap.downloadEmails.mockRejectedValue(error);

      await expect(Imap.downloadEmails(options)).rejects.toThrow('Download failed');
    });
  });
